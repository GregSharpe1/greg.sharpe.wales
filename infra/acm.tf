...
resource aws_acm_certificate dev_greg_sharpe_wales {
  provider = aws.us-east-1

  domain_name       = "dev-greg.sharpe.wales"
  validation_method = "DNS"
...
resource "aws_route53_record" "dev_greg_sharpe_wales_cert_validation" {
  provider = aws.us-east-1

  zone_id = aws_route53_zone.sharpe_wales.id
  name    = aws_acm_certificate.dev_greg_sharpe_wales.domain_validation_options[0].resource_record_name
  type    = aws_acm_certificate.dev_greg_sharpe_wales.domain_validation_options[0].resource_record_type
  ttl     = 60

  records = [
    aws_acm_certificate.dev_greg_sharpe_wales.domain_validation_options[0].resource_record_value
  ]
}
